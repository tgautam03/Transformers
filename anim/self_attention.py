...
class MultiHeadAttention(Scene):
    def construct(self):
        ...
        box = SurroundingRectangle(VGroup(nn1_, nn2_, nn3_), color=WHITE)
        self.play(Create(box), Create(nn1_), Create(nn2_), Create(nn3_))
        self.wait(1)
        ...
        # Q2_ = Text("Q2", color=RED).scale(0.35).next_to(Q1_, DOWN)
        # K2_ = Text("K2", color=GREEN).scale(0.35).next_to(K1_, DOWN)
        # V2_ = Text("V2", color=BLUE).scale(0.35).next_to(V1_, DOWN)
